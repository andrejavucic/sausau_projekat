...
with open("analysis/metrics_table.txt", "w", encoding="utf-8") as f:
    ...
    f.write("="*80 + "\n\n")
    f.write(f"{'Model':<22} {'CV F1':>10} {'Acc':>8} {'Prec':>8} {'Rec':>8} {'F1':>8} {'ROC':>10}\n")
    f.write("-"*80 + "\n")
    for name, r in results.items():
    ...
    for i, (name, r) in enumerate(sorted_results, 1):
        f.write(f"{i}. {name}: F1={r['f1']:.4f}, Recall={r['recall']:.4f}, CV_F1={r['cv_score']:.4f}\n")
...
for i, (name, r) in enumerate(sorted_results, 1):
    ...
    print(f"{medal} {name}: F1={r['f1']:.4f}, Recall={r['recall']:.4f}, CV_F1={r['cv_score']:.4f}")
print()
...
print()
# Primarni: najveći recall (Logistic Regression)
best_recall_name = max(results, key=lambda x: results[x]['recall'])
joblib.dump(results[best_recall_name]['model'], "models/best_recall_model.pkl")
print(f"🏆 Model sa najvećim recallom ({best_recall_name}) sačuvan kao: models/best_recall_model.pkl")

# Alternativni: najstabilniji po F2 (Random Forest)
best_f2_name = max(results, key=lambda x: results[x]['f2'])
joblib.dump(results[best_f2_name]['model'], "models/best_f2_model.pkl")
print(f"🏆 Model sa najboljim F2 ({best_f2_name}) sačuvan kao: models/best_f2_model.pkl")
